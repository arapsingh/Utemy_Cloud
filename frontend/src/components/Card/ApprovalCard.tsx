...
const ApprovalCard: FC<ApprovalCardProps> = ({ approval }) => {
    const [hovered, setHovered] = useState(false);
    return (
        <div className="py-2">
            <div className="flex gap-2 tablet:gap-4 rounded-2xl border-gray-300 border shadow-md w-full h-fit">
                <div className="h-fit w-[20%] bg-gray-400 shrink-0">
                    <img src={approval.course_thumbnail} alt={"course_thumnail"} className="w-[200px] h-[140px]" />
                </div>
                <Link to={`/admin/course/${approval.course_slug}/`} className={`w-full`}>
                    <div
                    ...
                    >
                        <div className={`w-[90%] tablet:w-full p-2 transition-all duration-300`}>
                            <h2 className="w-full text-xl font-bold title-card-content ">{approval.course_title}</h2>
                            <p>
                                Yêu cầu phê duyệt khoá học vào ngày {convertDateFormat(approval.created_at.toString())}
                            </p>
                            <p
