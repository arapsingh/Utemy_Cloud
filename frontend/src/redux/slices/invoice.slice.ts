...
export const createInvoice = createAsyncThunk<Response<null>, { totalWithCoupon: number; discount: number, coupon_id: number | null; }, { rejectValue: Response<null> }>(
    "invoice/create",
    async ({ totalWithCoupon, discount, coupon_id }, ThunkAPI) => {
        try {
            const response = await apis.invoiceApis.createInvoice(totalWithCoupon, discount, coupon_id);
